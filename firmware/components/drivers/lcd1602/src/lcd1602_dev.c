...
static void cont(void* u, i2c_req_t r, i2c_evt_kind_t k, ca_status_t s)
{ (void)u; (void)r; (void)k; (void)s; }

static ca_status_t send_cmd(lcd1602_dev_t* d, uint8_t v)
{
    uint8_t b[2]={0x80,v};
    return d->i2c->submit_write(d->i2c->ctx, d->addr7, b, 2, NULL, cont, d, NULL);
}
static ca_status_t send_data(lcd1602_dev_t* d, const uint8_t* data, uint8_t n)
{
    uint8_t buf[1+32];
    if(n>32) n=32;
    buf[0]=0x40;
    memcpy(&buf[1],data,n);
    return d->i2c->submit_write(d->i2c->ctx, d->addr7, buf, 1+n, NULL, cont, d, NULL);
}

ca_status_t lcd1602_dev_init(lcd1602_dev_t* d, const lcd1602_dev_cfg_t* cfg, void (*on_ready)(void*), void* u)
{
    if(!d||!cfg||!cfg->i2c||!cfg->time) {
        return CA_EINVAL;
    }
    memset(d,0,sizeof(*d));
    ...
    d->st=LCD_READY;
    if (on_ready) on_ready(u);
    return CA_OK;
}

ca_status_t lcd1602_dev_set_ddram(lcd1602_dev_t* d, uint8_t addr)
{
    uint8_t b[2]={0x80,(uint8_t)(0x80|addr)};
    return d->i2c->submit_write(d->i2c->ctx, d->addr7, b, 2, NULL, NULL, NULL, NULL);
}

ca_status_t lcd1602_dev_write_bytes(lcd1602_dev_t* d, const uint8_t* bytes, uint8_t n)
{
    return send_data(d, bytes, n);
}
